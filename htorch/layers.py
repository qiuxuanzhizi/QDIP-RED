...
class QConv2d(nn.Module):
    ...
    def reset_parameters(self):
        ...
        self.j_weight     = nn.Parameter(j)
        self.k_weight     = nn.Parameter(k)
        self.weight = torch.cat([torch.cat([nn.Parameter(r), -nn.Parameter(i), -nn.Parameter(j),  -nn.Parameter(k)], dim=0),
                            torch.cat([nn.Parameter(i),  nn.Parameter(r), -nn.Parameter(k),   nn.Parameter(j)], dim=0),
                            torch.cat([nn.Parameter(j),  nn.Parameter(k),  nn.Parameter(r),  -nn.Parameter(i)], dim=0),
                            torch.cat([nn.Parameter(k), -nn.Parameter(j),  nn.Parameter(i),   nn.Parameter(r)], dim=0)], dim = 1)
...
class QLinear(nn.Module):
    ...
    def reset_parameters(self):
        ...
        self.weight = torch.cat([torch.cat([nn.Parameter(r), -nn.Parameter(i), -nn.Parameter(j),  -nn.Parameter(k)], dim=0),
                            torch.cat([nn.Parameter(i),  nn.Parameter(r), -nn.Parameter(k),  nn.Parameter(j)], dim=0),
                            torch.cat([self.j_weight,  nn.Parameter(k),  nn.Parameter(r),  -nn.Parameter(i)], dim=0),
                            torch.cat([nn.Parameter(k), -nn.Parameter(j),  nn.Parameter(i),   nn.Parameter(r)], dim=0)], dim = 1)
